...
int main(int argc,char **args)
{
    ...
    std::cout << "Time taken: " << elapsed_s4 << "ms" << std::endl;


	for (int i=0;i<SIZE(r2);i++) printf("%d\n",r2[i]);
	
	// Store results
	size_t pos = graph_file.find_last_of('.');
    string base_name = (pos == string::npos) ? graph_file : graph_file.substr(0, pos);
    string output_file = "shs_multiMethod_" + base_name + ".txt";
	ofstream fout(output_file);
	if (fout.is_open()) {
		fout << "HIS:\n";
		int limit = std::min(25, (int)r2.size());
		for (int i = 0; i < limit; ++i) {
			fout << r2[i] + 1;	// We store 1-indexed vertex ids here
			if (i != limit - 1) fout << ", ";
		}
		fout << "," << std::endl; // final comma like your example
		fout.close();
	} else {
