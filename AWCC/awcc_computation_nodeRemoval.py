...
def node_removal(G, pcnt_removal):
    ...
    nodes = list(G.nodes())
    to_remove = np.random.choice(nodes, size=how_many, replace=False)
    G.remove_nodes_from(to_remove)
    return G

def process_graph(graph_file_name, shs_lists, methods):
    pltname = graph_file_name.split(".")[0]
    pickle_filename = f"{pltname}awcc_lists.pkl"
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"AbsAWCCChange_node_removal_{pltname}.pdf", format='pdf', dpi=300)
    plt.show()
    return
